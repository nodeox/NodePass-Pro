...
SOURCE_INSTALL="false" # false: 仅部署清单（默认，不保留源码） / true: 保留源码部署
...
引导参数:
  ...
  --with-source             保留完整仓库源码到安装目录（默认不保留）
  --build-image             透传给 deploy.sh，强制本地构建镜像（默认使用预构建镜像）
  --build-nodeclient        透传给 deploy.sh，构建 nodeclient 下载包（需配合 --with-source）
...
示例:
  ...
  # 保留源码部署（默认不保留源码）
  bash <(curl -fsSL https://raw.githubusercontent.com/nodeox/NodePass-Pro/main/install.sh) --with-source

  # 非交互，强制本地构建镜像（默认不构建）
  bash <(curl -fsSL https://raw.githubusercontent.com/nodeox/NodePass-Pro/main/install.sh) --non-interactive --build-image --license-key NP-XXXX-XXXX

  # 保留源码并构建 nodeclient 下载包
  bash <(curl -fsSL https://raw.githubusercontent.com/nodeox/NodePass-Pro/main/install.sh) --non-interactive --with-source --build-nodeclient --license-key NP-XXXX-XXXX
...
prepare_runtime_bundle() {
  ...
  log_info "使用镜像部署模式，拉取最小部署清单（不保留源码）..."
  git clone --branch "${BRANCH}" --depth 1 "${REPO_URL}" "${tmp_dir}" >/dev/null 2>&1

  if run_fs test -d "${INSTALL_DIR}"; then
    local keep_path
    for keep_path in ".env" "backend/configs" "deploy/caddy/Caddyfile" ".nodepass-version" ".nodepass-license"; do
      if run_fs test -e "${INSTALL_DIR}/${keep_path}"; then
        copy_path_if_exists "${INSTALL_DIR}/${keep_path}" "${backup_dir}/${keep_path}"
      fi
    done
  fi

  run_fs rm -rf "${INSTALL_DIR}"
  run_fs mkdir -p "${INSTALL_DIR}"
  ...
  for bundle_path in "${bundle_paths[@]}"; do
    copy_path_if_exists "${tmp_dir}/${bundle_path}" "${INSTALL_DIR}/${bundle_path}"
  done

  local restore_path
  for restore_path in ".env" "backend/configs" "deploy/caddy/Caddyfile" ".nodepass-version" ".nodepass-license"; do
    if run_fs test -e "${backup_dir}/${restore_path}"; then
      run_fs rm -rf "${INSTALL_DIR}/${restore_path}"
      copy_path_if_exists "${backup_dir}/${restore_path}" "${INSTALL_DIR}/${restore_path}"
    fi
  done
...
ensure_existing_installation() {
  if ! run_fs test -f "${INSTALL_DIR}/scripts/deploy.sh"; then
