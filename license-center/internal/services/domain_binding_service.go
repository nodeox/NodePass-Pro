...
func (s *DomainBindingService) matchDomain(license *models.LicenseKey, requestDomain string) bool {
	// 精确匹配
	if license.BoundDomain == requestDomain {
		return true
	}

	// 支持通配符 *.example.com
	if strings.HasPrefix(license.BoundDomain, "*.") {
		baseDomain := strings.TrimPrefix(license.BoundDomain, "*.")
