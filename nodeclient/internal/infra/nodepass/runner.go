...
// MaxAutoRestartAttempts 返回最大自动恢复次数（测试用）。
func MaxAutoRestartAttempts() int {
	return maxAutoRestartAttempts
}
...
// InstanceOption 用于测试辅助构造。
type InstanceOption func(*Instance)

// WithStatus 设置实例状态（测试用）。
func WithStatus(status string) InstanceOption {
...
// WithChannels 设置 done/stop channel（测试用）。
func WithChannels(doneCh, stopCh chan struct{}) InstanceOption {
	return func(instance *Instance) {
		if instance != nil {
			instance.doneCh = doneCh
			instance.TestDoneCh = doneCh
			instance.stopCh = stopCh
		}
	}
}

// WithDoneChannel 设置 done channel 状态（测试用）。
func WithDoneChannel(doneCh chan struct{}, closed bool) InstanceOption {
	return func(instance *Instance) {
		if instance != nil {
			instance.doneCh = doneCh
			instance.TestDoneCh = doneCh
			instance.doneClosed = closed
		}
	}
}

// WithStopChannel 设置 stop channel 状态（测试用）。
func WithStopChannel(stopCh chan struct{}, stopping bool) InstanceOption {
	return func(instance *Instance) {
		if instance == nil {
			return
		}
		instance.stopCh = stopCh
		instance.stopFlag = stopping
		if !stopping || stopCh == nil {
			return
		}
...
// NewInstanceForTest 创建测试用实例。
func NewInstanceForTest(ruleID int, rule config.RuleConfig, opts ...InstanceOption) *Instance {
	instance := &Instance{
		RuleID: ruleID,
		rule:   rule,
	}
	...
	if instance.doneCh != nil {
		instance.TestDoneCh = instance.doneCh
	}
	return instance
}
...
// InstanceForTest 注入实例（测试用）。
func (i *Integration) InstanceForTest(instance *Instance) {
	if i == nil || instance == nil {
		return
	}
	i.mu.Lock()
	defer i.mu.Unlock()
	i.instances[instance.RuleID] = instance
}

// InstanceExistsForTest 查询实例是否存在（测试用）。
func (i *Integration) InstanceExistsForTest(ruleID int) bool {
	if i == nil {
		return false
	}
	i.mu.RLock()
	defer i.mu.RUnlock()
	_, exists := i.instances[ruleID]
	return exists
}
...
func (i *Integration) GetAllStatus() []rules.RuleStatus {
	i.mu.RLock()
	defer i.mu.RUnlock()

	result := make([]rules.RuleStatus, 0, len(i.instances))
	for ruleID, instance := range i.instances {
		instance.mu.RLock()
		item := rules.RuleStatus{
			RuleID: ruleID,
			Mode:   instance.Mode,
...
func (i *Integration) GetTrafficStats() []rules.TrafficStat {
	i.mu.RLock()
	defer i.mu.RUnlock()

	stats := make([]rules.TrafficStat, 0, len(i.instances))
	for ruleID, instance := range i.instances {
		instance.mu.Lock()
		...
		stats = append(stats, rules.TrafficStat{
			RuleID:     ruleID,
...
func (i *Integration) MarkTrafficReported(reported []rules.TrafficStat) {
	if len(reported) == 0 {
		return
	}
...
// WatchProcessExitForTest 暴露给测试用（保持逻辑一致）。
func WatchProcessExitForTest(i *Integration, instance *Instance) {
	if i == nil || instance == nil {
		return
	}
	i.watchProcessExit(instance)
}
...
// HandleRestartFailureForTest 暴露给测试用（保持逻辑一致）。
func HandleRestartFailureForTest(i *Integration, instance *Instance, attempt int, err error) bool {
	if i == nil {
		return false
	}
	return i.handleRestartFailure(instance, attempt, err)
}
...
// RestartBackoffForTest 暴露给测试用（保持逻辑一致）。
func RestartBackoffForTest(attempt int) time.Duration {
...
// BuildCommandForTest 暴露给测试用（保持逻辑一致）。
func BuildCommandForTest(rule config.RuleConfig) (*exec.Cmd, error) {
	return buildCommand(rule)
}
