...
import (
	...
	domainheartbeat "nodepass-pro/nodeclient/internal/domain/heartbeat"
...
type heartbeatResponseEnvelope struct {
	...
	Data      domainheartbeat.HeartbeatResponseData `json:"data"`
	Message   string                               `json:"message"`
...
type ConfigUpdateHandler func(cfg *domainconfig.NodeConfig, version int)
...
func (h *HeartbeatService) Report() error {
	...
	payload := domainheartbeat.HeartbeatPayload{
...
// ApplyReportedConfigVersionForTest 暴露给测试用（保持逻辑一致）。
func ApplyReportedConfigVersionForTest(h *HeartbeatService, version int) {
	if h == nil {
		return
	}
	h.applyReportedConfigVersion(version)
}
...
func (h *HeartbeatService) collectHeartbeatData() (domainheartbeat.SystemInfoData, domainheartbeat.TrafficData) {
	...
	sysInfo := domainheartbeat.SystemInfoData{
	...
	traffic := domainheartbeat.TrafficData{
		TrafficIn:         trafficIn,
...
// ParseHeartbeatResponseForTest 暴露给测试用（保持逻辑一致）。
func ParseHeartbeatResponseForTest(body []byte) (domainheartbeat.HeartbeatResponseData, error) {
	return parseHeartbeatResponse(body)
}

func parseHeartbeatResponse(body []byte) (domainheartbeat.HeartbeatResponseData, error) {
	...
	if err := json.Unmarshal(body, &raw); err == nil {
		...
		if hasSuccess || hasData || hasMessage || hasError {
			...
			if err := json.Unmarshal(body, &envelope); err != nil {
				return domainheartbeat.HeartbeatResponseData{}, fmt.Errorf("解析心跳响应失败: %w", err)
			}
			if !envelope.Success {
				if envelope.Error != nil && envelope.Error.Message != "" {
					return domainheartbeat.HeartbeatResponseData{}, fmt.Errorf("心跳接口失败: %s (%s)", envelope.Error.Message, envelope.Error.Code)
				}
				if strings.TrimSpace(envelope.Message) != "" {
					return domainheartbeat.HeartbeatResponseData{}, fmt.Errorf("心跳接口失败: %s", strings.TrimSpace(envelope.Message))
				}
				return domainheartbeat.HeartbeatResponseData{}, fmt.Errorf("心跳接口失败: success=false")
			}
			return envelope.Data, nil
		}
	}

	var direct domainheartbeat.HeartbeatResponseData
