		...
		Rules: []domainconfig.RuleConfig{
			{
				...
				Mode:     "single",
				Protocol: "tcp",
				Listen:   domainconfig.HostPort{Host: "0.0.0.0", Port: 8080},
				Target:   domainconfig.HostPort{Host: "127.0.0.1", Port: 9090},
			},
		},
...
func TestConfigCacheLoadNonExistent(t *testing.T) {
	...
	if err != config.ErrCacheNotFound {
...
func TestConfigCacheAtomicWrite(t *testing.T) {
	...
	// 保存初始配置
	config1 := &domainconfig.NodeConfig{
		ConfigVersion: 1,
		Rules:         []domainconfig.RuleConfig{},
		Settings: domainconfig.Settings{
			HeartbeatInterval:   30,
	...
	// 保存新配置（应该原子替换）
	config2 := &domainconfig.NodeConfig{
		ConfigVersion: 2,
		Rules:         []domainconfig.RuleConfig{},
		Settings: domainconfig.Settings{
			HeartbeatInterval:   30,
...
func TestConfigCacheNilCache(t *testing.T) {
	var cache *config.ConfigCache

	// 所有方法应该安全处理 nil
	err := cache.Save(&domainconfig.NodeConfig{})
	if err == nil {
