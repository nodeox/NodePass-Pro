...
// Verify 执行一次统一授权校验。
func (v *Verifier) Verify(currentVersion string) (*VerifyStatus, error) {
	if !v.Enabled() {
		return &VerifyStatus{Allowed: true, Message: "license check disabled", Status: "disabled"}, nil
	}
	...
	if err != nil {
		if v.cfg.LicenseFailOpen {
			return &VerifyStatus{Allowed: true, Message: "授权中心不可达，license_fail_open 生效", Status: "fail_open"}, nil
		}
	...
	if err = json.NewDecoder(resp.Body).Decode(&parsed); err != nil {
		if v.cfg.LicenseFailOpen {
			return &VerifyStatus{Allowed: true, Message: "授权响应解析失败，license_fail_open 生效", Status: "fail_open"}, nil
		}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		if v.cfg.LicenseFailOpen {
			return &VerifyStatus{Allowed: true, Message: msg + "（license_fail_open）", Status: "fail_open"}, nil
		}
