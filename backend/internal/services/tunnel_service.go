...
func (s *TunnelService) Update(userID uint, id uint, req *UpdateTunnelRequest) (*models.Tunnel, error) {
	...
	if req.Config != nil {
		if err = validateTunnelConfig(nextProtocol, req.Config); err != nil {
			return nil, err
		}
...
func validateTunnelConfig(protocol string, cfg *models.TunnelConfig) error {
	if cfg == nil {
		return nil
	}
	...
	if cfg.ProtocolConfig != nil {
		if err := validateProtocolConfig(protocol, cfg.ProtocolConfig); err != nil {
			return err
		}
	}
...
// validateProtocolConfig 按协议验证协议特定配置。
func validateProtocolConfig(protocol string, cfg *models.ProtocolConfig) error {
	if cfg == nil {
		return nil
	}
	...
	case "tls":
		if cfg.TLSVersion != nil {
			version := strings.ToLower(strings.TrimSpace(*cfg.TLSVersion))
			validVersions := map[string]bool{
				"tls1.2": true,
				"tls1.3": true,
			}
			if !validVersions[version] {
				return fmt.Errorf("%w: tls_version 必须是 tls1.2/tls1.3", ErrInvalidParams)
			}
		}
		return nil
